...
namespace TiltBrush
{
    ...
    public class ScreenshotManager : MonoBehaviour
    {
        ...
        /*
        static public async Task Save(Stream outf, RenderTexture rTextureToSave, bool bSaveAsPng)
        {
            var buffer = SaveToMemory(rTextureToSave, bSaveAsPng);
            Stream stream = new MemoryStream(buffer);

            //여기서 buffer 가 png로 변한 캡쳐이미지이다.

            PredictResults res;
            var client = new GoogleAutoMLVisionClient("‪C:/ssafy/circular-hybrid-312909-c7f989c7bd7a.json");
            res = await client.Predict("us-central1", stream);
            Console.WriteLine($"Your image is a {res}");
        ...
        static public async Task MlVision(byte[] buffer)
        {
            Stream stream = new MemoryStream(buffer);
            ...
            var requestBody = new
            {
                payload = new
                {
                    image = new
                    {
                        imageBytes = Utils.StreamToBase64(stream)
                    }
                }
            };
            ...
            using (var client = new HttpClient())
            {
                ...
                try
                {
                    var result = await client.PostAsync("http://k4a101.p.ssafy.io:3000/image", content);
                ...
                Debug.Log(strResult);
                ScreenshotManager scm = new ScreenshotManager();

                //태극기가 아닐때는 strResult.Length가 11임
                if (strResult.Length < 9)
                {

                    Debug.Log("그리기 성공");
                    GameObject successParent = GameObject.FindGameObjectWithTag("SuccessParent");
                    GameObject success = successParent.transform.Find("Success").gameObject;
                    success.SetActive(true);
                    // ob1.GetComponent<SuccessScript>().show();
                    success.GetComponent<SuccessScript>().show();
                }
                else
                {
                    ...
                    Debug.Log("그리기 실패");
                    GameObject successParent = GameObject.FindGameObjectWithTag("SuccessParent");
                    GameObject fail = successParent.transform.Find("fail").gameObject;
                    fail.SetActive(true);
                    // ob1.GetComponent<SuccessScript>().show();
                    fail.GetComponent<FailScript>().show();
                }
            }
